...
// // const JumperCanvas: React.FC<{ onWin: () => void }> = ({ onWin }) => {
...
// //       // Physics
// //       stateRef.current.vy += 0.4; // Gravity
// //       stateRef.current.playerY += stateRef.current.vy;

// //       // Horizontal Movement (Smooth Lerp)
// //       stateRef.current.playerX +=
// //         (stateRef.current.vx - stateRef.current.playerX) * 0.1;

// //       // Screen Wrapping
// //       if (stateRef.current.playerX < -20)
// //         stateRef.current.playerX = canvas.width;
// //       if (stateRef.current.playerX > canvas.width)
// //         stateRef.current.playerX = -20;

// //       // Camera Movement (Only go up)
// //       if (stateRef.current.playerY < canvas.height / 2) {
// //         const diff = canvas.height / 2 - stateRef.current.playerY;
// //         stateRef.current.playerY = canvas.height / 2;
// //         stateRef.current.score += Math.floor(diff);

// //         // Move platforms down
// //         stateRef.current.platforms.forEach((p) => (p.y += diff));
...
// //       // Bounce Collision (Only when falling)
// //       if (stateRef.current.vy > 0) {
// //         stateRef.current.platforms.forEach((p) => {
// //           if (
// //             stateRef.current.playerX + 20 > p.x &&
// //             stateRef.current.playerX < p.x + p.w &&
// //             stateRef.current.playerY + 30 > p.y &&
// //             stateRef.current.playerY + 30 < p.y + 15
// //           ) {
// //             stateRef.current.vy = -10; // Jump force
// //           }
// //         });
// //       }

// //       // Game Over (Fall off screen)
// //       if (stateRef.current.playerY > canvas.height) {
// //         // Simple respawn logic for endless fun (or could trigger lose)
// //         stateRef.current.vy = -15;
// //         stateRef.current.playerY = canvas.height - 100;
// //         stateRef.current.score = Math.max(0, stateRef.current.score - 500);
// //       }

// //       // Win Condition
// //       if (stateRef.current.score >= 2000 && !stateRef.current.won) {
...
// //       stateRef.current.platforms.forEach((p) => {
// //         ctx.beginPath();
// //         if (ctx.roundRect) ctx.roundRect(p.x, p.y, p.w, 10, 5);
...
// //       ctx.fillText("🚀", stateRef.current.playerX, stateRef.current.playerY);

// //       // HUD
// //       ctx.fillStyle = "white";
// //       ctx.font = "bold 20px monospace";
// //       ctx.textAlign = "left";
// //       ctx.fillText(`Altitude: ${stateRef.current.score}`, 20, 40);
// //       ctx.font = "14px sans-serif";
// //       ctx.fillText(`Target: 2000`, 20, 60);
...
// //       stateRef.current.vx = x;
// //     };

// //     const onMove = (e: MouseEvent) => handleInput(e.clientX);
...
//   {
//     totalTime: number;
//     spawnRate: number; // orbs per second
//     orbSpeedMin: number;
//     orbSpeedMax: number;
//     orbValue: number;
//     rocketLerp: number; // 0–1, how fast rocket follows cursor
//   }
// > = {
//   Easy: {
//     totalTime: 55,
...
//       const lerp = settings.rocketLerp;
...
//         ctx.globalAlpha = 0.4 + 0.4 * twinkle;
...
//         cancelAnimationFrame(animationFrameRef.current);
//     };
//   }, [difficulty, onWin, onLose, settings.totalTime]);
...
//       <canvas
//         ref={canvasRef}
//         className="w-full h-full block rounded-3xl bg-transparent"
//       />
//     </div>
//   );
// };
